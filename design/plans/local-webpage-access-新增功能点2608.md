...
> **状态（2026-08-11）**：Gate-A 静态预检、IMP-057 Monorepo 分类和 Gate-B 扁平多候选识别已有实现记录（DEV-106～108）；IMP-056 兼容性预检已有实现记录（DEV-109）。DEV-110 记录的是评审前的初版 Gate-C 实现；CHK-190 / DOC-129 已明确该版本不能代表修订架构最终验收。修订版 Gate-C 的能力契约、成功谓词、等价 fallback 与外部副作用回滚由 **DEV-111（待开发）**重新承接。
>
...
| Layer 1 | 从证据生成完整部署计划，而不是遇到首个信号就结束 | `DeploymentPlan[]` |
...
| --- | --- | --- |
| DEV-106 / Gate-A | Layer 2 静态预检已实现 | 保留为战术护栏；不得扩大字符串 shell/SQLite 自动改写范围 |
...
| DEV-110 / 初版 Gate-C | 候选降级、诊断和 API 探测已有实现记录 | **需按修订架构重新验收/返工**；不得据“已完成”状态推断能力守恒和副作用回滚已经满足 |
...
| ID | 关系 |
| --- | --- |
| `IMP-056` / `DEV-109` | 兼容性预检 |
| `IMP-057` / `DEV-107` | Monorepo 分类与主包选择 |
| `IMP-058` / `DEV-106` / `DEV-108` | Scanner Gate-A/B |
| `DEV-110` | 评审前初版 Gate-C 实现记录；修订版 Gate-C 尚未据此验收 |
| `DEV-111` | 修订版 Gate-C 待开发项；承接能力契约、成功谓词、事务回滚和等价 fallback |
| `CHK-189` / `PLN-039` | 前置预检与 Monorepo 设计评审/规划 |
| `CHK-190` / `DOC-128` / `DOC-129` | Scanner 与路径复盘评审、设计文档及修订记录 |
