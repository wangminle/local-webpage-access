"""应用版本解析：优先从 Git 最新 commit 主题读取 ``V0.8.10-Build...`` 前缀。"""
...
def resolve_version() -> str:
    """返回 semver 字符串（如 ``0.8.10``），不含 ``V`` 前缀。"""
...
def version_from_subject(subject: str | None) -> str | None:
    """从 commit 主题解析 ``V0.8.10-Build...`` 前缀（IMP-063）。
