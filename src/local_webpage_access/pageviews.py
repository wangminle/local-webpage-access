...
from local_webpage_access.ports import detect_local_ips, is_local_ip
...
_DDL_STATEMENTS: tuple[str, ...] = (
    ...
    """,
    "CREATE INDEX IF NOT EXISTS idx_detail_instance ON pageview_detail(instance_id, ts)",
    """
    ...
    """,
    """
    -- IMP-026：全量 IP 聚合真相表。pageview_detail 只留最近 N 条，不能当全量统计源；
    -- 本表随每个 page hit 在同一事务 UPSERT，保证"独立 IP 列表"的次数与最近时间完整。
    CREATE TABLE IF NOT EXISTS pageview_ip_stats (
        instance_id TEXT NOT NULL,
        remote TEXT NOT NULL,
        hits INTEGER NOT NULL DEFAULT 0,
        last_seen TEXT,
...
class AccessHit:
    ...
    status: int
    remote: str
    host: str = ""  # Caddy request.host（含端口），IMP-028 无别名直连端口归属用
...
def _is_page_view(method: str, path: str, status: int) -> bool:
    """判定一条命中是否为「页面访问」（IMP-025.a）。

    保守启发式（CLF 无 Content-Type / Sec-Fetch-Dest）：

    1. 仅 ``GET`` 计数。``HEAD`` 通常来自探活/预检，不视为一次页面打开。
    2. 仅计正常文档响应：``200–299``（排除 ``204/206``）与缓存复用 ``304``；
       重定向/错误不计，避免一次导航的 ``3xx → 200`` 被算两次。
    3. 拆 query；查询参数含保留标记 ``__lwa_probe`` 时排除（LWA 自身探针）。
    4. 末段小写后缀命中 :data:`_ASSET_EXT` 则排除（资源文件）。
    5. 排除常见非页面端点（精确 ``/api`` 等 + ``/api/`` 前缀）。
    6. 其余视为 page（``/``、``/index.html``、``/about``、SPA 无扩展路由）。
    """
    if method.upper() != "GET":
        return False
    if status != 304 and (status < 200 or status > 299 or status in (204, 206)):
        return False
    parts = urlsplit(path)
    ...
    if last_seg:
        dot = last_seg.rfind(".")
        if dot != -1 and last_seg[dot:].lower() in _ASSET_EXT:
            return False
    return True


def _has_probe_marker(query: str) -> bool:
    """query 串中是否含保留参数 ``__lwa_probe``（任意取值）。"""
    for pair in query.split("&"):
        k, _sep, _v = pair.partition("=")
        if k == "__lwa_probe":
            return True
    return False
...
class PageviewStore:
        ...
        with self._lock:
            if self._conn is None:
                ...
                try:
                    ...
                    conn.close()
                    raise
                # schema 初始化成功后才能缓存；失败连接必须丢弃，确保后续访问
                # 仍会执行版本保护（BUG-134）。
                self._conn = conn
            return self._conn
    ...
    def _init_schema(self, conn: sqlite3.Connection) -> None:
        ...
        ``connect()`` 用 ``isolation_level=None``（手动事务），故用显式
    ...
    def record_hits(
        ...
        即被 :func:`_is_page_view` 过滤，不进入 ``pageviews.hits`` /
        ``pageview_ips`` / ``pageview_detail`` / ``pageview_ip_stats``，使累计访问、
        独立 IP、按天分布、最近命中、IP 列表全部一致为 page 口径。
        ...
        IMP-026：每个 page hit 同时 UPSERT ``pageview_ip_stats``（全量 IP 聚合真相
        表），与上述写入在同一事务内完成。
        """
        page_hits = [h for h in hits if _is_page_view(h.method, h.path, h.status)]
        if not page_hits:
            return 0
        conn = self._conn_or_open()
        ...
        with self._lock:
            ...
            try:
                ...
                # IMP-026：全量 IP 聚合（次数累加，最近时间取大）
                if per_remote:
                    conn.executemany(
                        "INSERT INTO pageview_ip_stats(instance_id, remote, hits, "
                        "last_seen) VALUES(?,?,?,?) "
                        "ON CONFLICT(instance_id, remote) DO UPDATE SET "
                        "hits=pageview_ip_stats.hits+excluded.hits, "
                        "last_seen=MAX(pageview_ip_stats.last_seen, excluded.last_seen)",
                        [
                            (instance_id, remote, st["hits"], st["last"])
                            for remote, st in per_remote.items()
                        ],
                    )
                conn.executemany(
    ...
    def record_file_batch(
        self,
        per_instance: dict[str, list[AccessHit]],
        source: str,
        cursor_key: str,
        next_offset: int,
    ) -> int:
        ...
        with self._lock:
            conn.execute("BEGIN IMMEDIATE")
            try:
                for instance_id, hits in per_instance.items():
                    total += self.record_hits(instance_id, source, hits)
                conn.execute(
                    "INSERT INTO ingest_cursor(source_key, offset_bytes, last_ts, updated_at) "
                    "VALUES(?,?,?,?) "
                    "ON CONFLICT(source_key) DO UPDATE SET "
    ...
    def record_container_batch(
        self,
        instance_id: str,
        pairs: list[tuple[str, AccessHit]],
        cursor_key: str,
    ) -> int:
        """原子完成容器源切换、日志去重、聚合写入与游标推进。

        这样既避免 Caddy→docker logs 的跨源重复计数（BUG-132），也避免
        ``container_seen`` 先提交而聚合失败导致日志永久丢失（BUG-133）。
        """
        if not pairs:
            return 0
        conn = self._conn_or_open()
        new_hits: list[AccessHit] = []
        with self._lock:
            conn.execute("BEGIN IMMEDIATE")
            try:
                ...
                if changed is not None:
                    self._delete_instance_rows(conn, instance_id)
                ...
                total = self.record_hits(instance_id, "container", new_hits)
                if new_hits:
                    conn.execute(
                        "INSERT INTO ingest_cursor(source_key, offset_bytes, last_ts, "
                        "updated_at) VALUES(?,?,?,?) "
                        "ON CONFLICT(source_key) DO UPDATE SET "
                        "offset_bytes=excluded.offset_bytes, last_ts=excluded.last_ts, "
                        "updated_at=excluded.updated_at",
    ...
    def summary(self) -> dict[str, dict[str, Any]]:
        ...
        回访会被重复算），改用 ``COUNT(DISTINCT remote)`` 得到真正的实例级独立
        访客数。

        IMP-026 一致性：``uniqueIps`` 改从 ``pageview_ip_stats`` 取行数，与
        :meth:`detail` 返回的 ``uniqueIpList`` 长度定义一致。
        """
        conn = self._conn_or_open()
        with self._lock:
            rows = conn.execute(
    ...
    def detail(self, instance_id: str, *, limit: int = 50) -> dict[str, Any]:
        """单实例详情：按天分布 + 最近明细行 + 数据来源 + 全量独立 IP 列表。"""
        conn = self._conn_or_open()
        with self._lock:
            ...
            ip_rows = conn.execute(
                "SELECT remote, hits, last_seen FROM pageview_ip_stats "
                "WHERE instance_id=? ORDER BY hits DESC, last_seen DESC, remote ASC",
                (instance_id,),
            ).fetchall()
        # IMP-026：本机地址探测只做一次（会派生 tailscale 子进程），不逐 IP 重复
        local_ips = detect_local_ips() if ip_rows else set()
            ...
            "uniqueIpList": [
                {
                    ...
                    "count": int(r[1] or 0),
                    "lastSeen": r[2],
                    "local": is_local_ip(r[0], local_ips) if r[0] else False,
                }
                for r in ip_rows
            ],
        }
    ...
    @staticmethod
    def _delete_instance_rows(conn: sqlite3.Connection, instance_id: str) -> None:
        """在调用方事务内删除单实例的聚合、明细、去重集与独占游标。"""
...
def _instance_sources(
    ...
    for row in registry.list_instances():
        ...
        if runtime == "docker-compose":
            # IMP-027：有 Caddy 别名的容器走 caddy 共享日志（真实访客 IP +
            # page 过滤 + __lwa_probe 排除 + 本机标记）；无别名 / 非 caddy 后端
            # 才回退 docker logs（IP 常为网关，近似值）。一实例一源，避免双计。
            alias = _instance_alias(registry, iid)
            if backend == "caddy" and alias:
                sources.append(_InstanceSource(iid, "caddy", alias=alias))
            else:
            ...
            alias = _instance_alias(registry, iid)
            # IMP-028：无别名的静态站点走 Caddy 直连端口，按 host 端口归属访问量
            port = _static_host_port(registry, iid) if kind == "caddy" else None
            sources.append(_InstanceSource(iid, kind, alias=alias, host_port=port))
    return sources
...
def _instance_alias(registry: Registry, instance_id: str) -> str | None:
    """实例的路径别名 slug，静态与容器通用（IMP-027 扩展容器）。

    静态实例取 ``static_sites.route_host``（保持既有行为）；未命中再查
    ``containers`` 表，取 ``route_mode='name'`` 的 ``route_host``。容器别名是
    IMP-027 让容器改走 Caddy 日志统计的依据。
    """
    try:
        ...
        row = None
    if row:
        alias = str(row.get("route_host") or row.get("path_alias") or "")
        if alias:
            return alias
    # IMP-027：容器实例的别名（仅 name 模式才有 /<alias>/ 路由）
    try:
        crow = registry.get_container(instance_id)
    except Exception:  # noqa: BLE001
        crow = None
    if crow and crow.get("route_mode") == "name":
        return str(crow.get("route_host") or "") or None
    return None


def _static_host_port(registry: Registry, instance_id: str) -> int | None:
    """静态实例对外端口（``static_sites.host_port``）。

    IMP-028：无路径别名的静态站点由 Caddy 直连端口伺服，其访问日志的
    ``request.host`` 含端口，据此把访问归属到实例。
    """
    try:
        row = registry.get_static_site(instance_id)
    except Exception:  # noqa: BLE001
...
def ingest_all(
    ...
    with _ingest_lock:
        ...
        port_to_id: dict[int, str] = {}

        # 1. Caddy 共享 access log（统一入口块 :8080 + 直连端口站点写入）
        #    有别名 → 按 /<alias>/ 前缀归属；无别名直连端口 → 按 host 端口归属（IMP-028）
...
def _port_from_host(host: str) -> int | None:
    """从 Caddy ``request.host`` 取端口（如 10.0.0.1:18000 / [::1]:18000）；无则 None。"""
...
def _ingest_caddy_shared(
    ...
    """解析 Caddy 共享 access log，按 ``/<alias>/`` 前缀或直连端口归属到实例。
    ...
    for ln in batch.lines:
        ...
        if matched_id is None and port_to_id:
            # IMP-028：无别名的直连端口站点，按 request.host 端口归属
            port = _port_from_host(hit.host)
            if port is not None and port in port_to_id:
                matched_id = port_to_id[port]
...
def _ingest_container(
    ...
    if not pairs:
        return
    store.record_container_batch(src.instance_id, pairs, cursor_key)
