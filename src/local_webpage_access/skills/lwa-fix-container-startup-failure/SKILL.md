...
1. `apps/<id>/logs/run.log`（容器 stdout/stderr）。
2. `apps/<id>/docker/compose.yaml` / `apps/<id>/docker/Dockerfile`。
3. `local-web.json`（`container`、`entry.start`、资源限制）。
4. registry 的 `last_error`。

## 输出

- 修正后的 `Dockerfile` CMD / `compose.yaml` command / `entry.start`。
- 资源限制调整建议（若 OOM）。
- 诊断说明（事件日志）。
...
1. 读 `run.log`，归类：
   ...
   - **OOM**：内存限制过低，或进程本身泄漏。
   - **权限**：非 root 用户写不了 `/app/data`、绑定 <1024 端口。
2. 针对性修复：
   - 缺环境变量 → 在 compose `environment` / `.env` 补齐（默认值 + 提示修改）。
...
`run.log` 含 `permission denied: /app/data/app.db` → 非 root 用户无写权限：
