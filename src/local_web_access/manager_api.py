...
* **复用 lifecycle**：start/stop/restart/rebuild 直接调用
  :mod:`local_web_access.lifecycle` 的同名函数，确保管理页操作与 CLI 走同一套
  逻辑（验收标准 3）。
* **WAL + 单连接**：registry 在 manager 进程内打开一个连接，借助 SQLite WAL
  与 :func:`~local_web_access.registry.connection.transaction` 的连接级锁，
...
def create_app(
    ...
    *,
    ...
    from local_web_access.logging import setup_logging
    ...
    app = FastAPI(
        title="Local Web Access Manager",
...
_PLACEHOLDER_HTML = (
    ...
    "<title>Local Web Access</title></head>"
    "<body style='font-family:system-ui;padding:2rem;line-height:1.6'>"
    "<h1>Local Web Access Manager</h1>"
