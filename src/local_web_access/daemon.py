...
3. 调用 :class:`~local_web_access.importer.Importer` 自动导入（WBS-21.07）；
4. 对**可确定且非重型**的实例（tiny/small，非 pending）调用
   :func:`~local_web_access.lifecycle.start_instance` 自动启动（WBS-21.08）；
...
# ---- CLI 入口（``python -m local_web_access.daemon --workspace ...``）-------
