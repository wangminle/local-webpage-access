...
def setup_logging(
    ...
    *,
    ...
    root = logging.getLogger("local_web_access")
    root.setLevel(level)
...
def get_logger(name: str) -> logging.Logger:
    """获取子模块 logger，自动归入 ``local_web_access`` 命名空间。"""
    if name.startswith("local_web_access"):
        return logging.getLogger(name)
    return logging.getLogger(f"local_web_access.{name}")
