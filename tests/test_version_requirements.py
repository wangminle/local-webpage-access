...
def test_version_ge_and_gt() -> None:
    ...
    assert version_ge(MIN_CADDY_VERSION, MIN_CADDY_VERSION)
    assert version_ge("2.11.3", MIN_CADDY_VERSION)
    assert not version_ge("2.9.9", MIN_CADDY_VERSION)
    assert version_ge("2.10.0", MIN_CADDY_VERSION)
