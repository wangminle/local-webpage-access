...
def test_resolve_version_prefers_git(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(version_info, "_version_from_git", lambda root: "0.5.1")
    monkeypatch.setattr(version_info, "_version_from_metadata", lambda: "0.4.4")
    assert version_info.resolve_version() == "0.5.1"
...
def test_resolve_version_uses_metadata_when_git_missing(
    ...
    monkeypatch.setattr(version_info, "_version_from_metadata", lambda: "0.5.1")
    assert version_info.resolve_version() == "0.5.1"
...
def test_display_version_prefix(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(version_info, "_version_from_git", lambda root: "0.5.1")
    assert version_info.display_version() == "V0.5.1"


def test_version_from_git_subject() -> None:
    assert version_info._version_from_git(None) is None
    match = version_info._VERSION_PREFIX.match("V0.5.1-Build0567-20260709")
    assert match is not None
    assert match.group(1) == "0.5.1"
