...
- **宿主机装配（IMP-031/032）**：`lwa setup` / `lwa init` 内置 macOS/Linux 的 Docker Engine+Compose、Caddy 安装脚本（默认国内源 + registry-mirrors）；`--default` 检测+指引（缺 Docker 时可询问安装），`--full` 检查并装齐 Caddy+Docker+Compose（非 TTY 需 `--yes`）。
...
安装完成后建议运行 `lwa setup` 检测宿主机工具（或 `lwa setup --full --yes` 一次装齐 Caddy/Docker/Compose），再 `lwa doctor` 做完整自检（需先 `lwa init`），详见 [排障指南](docs/faq.md) 与 [运维手册 · 宿主机装配](docs/operations-playbook.md#零宿主机装配imp-031032)。
...
# 可选：lwa setup --full --yes      # 检查并安装 Caddy + Docker Engine + Compose
...
| 命令 | 说明 |
| --- | --- |
| `lwa init [-w DIR] [--force] [--default\|--full] [--yes]` | 初始化工作区（目录 / 配置 / registry / skills），幂等；`--full` 额外装齐 Caddy/Docker/Compose |
...
| `lwa list` | 列出所有实例及端口 |
| `lwa setup [--script] [--json] [--default\|--full] [--yes] [--autostart] [--with-caddy]` | 检测宿主机工具；`--full` 安装缺失的 Caddy/Docker/Compose；`--autostart` 委托 `lwa autostart install` |
...
| [docs/operations-playbook.md](docs/operations-playbook.md) | 日常运维速查（宿主机装配、网关选型、inbox 规范、冗余清理、容器别名、Caddy 排障） |
