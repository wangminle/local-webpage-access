...
- **`lwa configure`：实例级配置（V0.8.13）** — 新命令（及 `PATCH /api/instances/{id}/settings`）查看、整组替换或清空 `buildEnv`；`--follow-alias-base` 每次构建按当前路径别名推导 `VITE_BASE`（无别名为 `/`，优先于手动值——项目须读取该变量，`lwa rebuild` 生效）；`--acknowledge-redundancy` 标记有意保留的重复实例并退出冗余候选（可撤销）。每个开关均有 `--no-` 反向形式；仅宿主前端构建，容器运行时拒绝。
