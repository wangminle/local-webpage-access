...
需要 Python 3.13+，以及 **fastapi ≥ 0.138.0**、**uvicorn ≥ 0.45.0**（见 `pyproject.toml`）。托管容器实例需要 **Docker ≥ 29.6.1** 与 **Docker Compose ≥ 5.2.0**；静态网关配置为 Caddy 时需 **Caddy ≥ 2.11.2**。生成的容器基线镜像为 `node:24-alpine` 与 `python:3.13-slim`。静态托管在无 Caddy 时自动使用内置服务。运行 `lwa doctor` 可逐项校验上述版本。
