...
- **排障辅助（WBS-26 / IMP-033/034）**：`lwa doctor` 检查 Python / Docker / Compose / 端口池 / registry / 磁盘 / 内存；`--profile full` / `lwa capabilities` 输出统一 CapabilityReport；CLI / manager / daemon / gateway 分文件落盘（`logs/lwa.log` 等），FAQ 提供症状→日志对照。
...
| `lwa doctor [ID] [--json] [--profile default\|full]` | 诊断环境与实例；`--profile full` 附加 CapabilityReport；`--json` 机器可读，有 fail / Full unready 时退出码 1 |
...
pending/failed/可恢复异常队列、端口池与统计。Docker/Caddy **能力降级**时顶部显示横幅，
并禁用容器启停按钮；后端对容器生命周期 API 同样 fail-closed（`capability_denied`）。
Caddy 模式下实例状态除运行中/已停止外，还会区分 **网关不可达**（`gateway_down`）与
**配置无效**（`config_invalid`），以及观测失败时的 **unknown**（权限不足等，不误写成 stopped）。
可点 **恢复**（先拉起 Caddy master 再 restart）。API 与鉴权见
