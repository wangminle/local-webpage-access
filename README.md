...
- **运行形态自动识别**：扫描 `package.json` / `requirements.txt` / `pyproject.toml` / `Pipfile` / `manage.py` 等，判定 `static` / `node` / `python` 与是否含数据库；识别失败标记 `pending` 并写入风险提示事件。**V0.7.6 / IMP-058**：五层流水线（证据收集→候选生成→静态预检→实证校验→收敛诊断）--Gate-A 静态预检（shell 操作符防护 CHK-V02、SQLite 绝对路径 DATABASE_URL CHK-V03、Alembic cwd 编排 CHK-V04、自定义 Dockerfile 警告 CHK-V06）；Gate-B 多候选生成与能力契约校验（含 Node 子目录候选、sourceSubdir 扁平复制）；Gate-C 实证校验状态机（VERIFYING → RUNNING / DEGRADED / FAILED，证据驱动探针 + 能力守恒 + 回滚边界 + 降级策略）。
