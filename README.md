...
- **端口池管理**：从配置端口池中分配，跳过 registry 已登记端口与宿主机实际监听端口，生成 `lanUrl` / `healthUrl`；静态站点可选**路径别名**（`/<slug>/` 统一入口，Caddy 模式）。
...
- **管理页（WBS-22/23）**：内置 HTTP API + 单页前端，token 鉴权，覆盖实例列表 / 详情 / 日志 / 资源 / 生命周期操作 / 路径别名 / pending 队列 / 端口池 / 统计。
...
- **大模型 Skills（WBS-24）**：15 个 SKILL.md 覆盖环境初始化、导入、托管、容器、生命周期、排障等场景，供 AI 编程助手协作。

## 安装

需要 Python 3.13+，以及 **fastapi ≥ 0.138.0**、**uvicorn ≥ 0.45.0**（见 `pyproject.toml`）。托管容器实例需要 **Docker ≥ 29.0.0** 与 **Docker Compose ≥ 2.40.2**（推荐 ≥ 5.2.0）；静态网关优先使用 Caddy（Caddy 模式需 **Caddy ≥ 2.11.2**），无 Caddy 时自动使用内置服务。生成的容器基线镜像为 `node:24-alpine` 与 `python:3.13-slim`。运行 `lwa doctor` 可逐项校验上述版本。
...
安装后提供 `lwa` 命令；也可用 `python -m local_webpage_access` 调用。
...
# 可选：静态站点路径别名（Caddy 模式下 http://<LAN-IP>:<staticGatewayPort>/<slug>/）
...
# 7.（可选）打开管理页或开启自动导入
lwa manager start             # 前台启动管理页，浏览器访问 http://<LAN-IP>:17800
lwa daemon on                 # 监听 inbox/，自动导入并启动轻量实例
...
管理页 token 在首次 `lwa manager start` 时生成并打印；也可在工作区 `run/` 下查看。详见 [管理页说明](docs/manager-page.md)。
...
| `lwa alias set <ID> <slug>` / `lwa alias clear <ID>` | 为静态实例设置或清除路径别名（与管理页/API 共用逻辑） |
...
staticGatewayPort: 17880   # 路径别名统一入口端口（Caddy 模式，需与 managerPort 错开）
...
<workspace>/
├─ local-web.yml            # 全局配置
├─ inbox/                   # 待导入的 zip（daemon 自动监听）
├─ registry/local-web.db    # SQLite registry
├─ static-gateway/sites/    # 静态站点网关配置
├─ run/                     # 运行期 PID / 锁文件 / 管理 token
├─ logs/                    # 全局日志
├─ skills/                  # 15 个大模型协作 SKILL.md（WBS-24）
└─ apps/<id>/
...
`lwa manager start` 启动内置管理页（FastAPI + 单页前端），默认监听 `0.0.0.0:17800`。
首次启动会生成访问 token 并打印到终端，浏览器打开后输入 token 即可使用。
管理页覆盖实例列表、详情、日志查看、资源占用、start/stop/restart/rebuild 操作、
静态实例路径别名设置、pending/failed 队列、端口池与统计。API 端点与鉴权细节见 [管理页说明](docs/manager-page.md)。

## 自动导入守护进程

`lwa daemon on` 开启 inbox/ 自动监听：把 zip 放进 `inbox/`，daemon 会自动导入，
对可确定的轻量实例（纯静态、已能识别的前端）直接启动。`lwa daemon off` 关闭，
`lwa daemon status` 查看运行状态。
...
| [docs/plan/待改进功能点记录-20260706.md](docs/plan/待改进功能点记录-20260706.md) | V1 后待改进功能 backlog |
...
详细设计见 `docs/plan/local-webpage-access-v1-design-20260704.md` 与 `docs/plan/local-webpage-access-v1-wbs-20260704.md`；任务跟踪见 `task-list.md`。
