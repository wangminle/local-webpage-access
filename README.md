...
- **大模型 Skills（WBS-24）**：12 个 SKILL.md 覆盖导入、托管、容器、生命周期、排障等场景，供 AI 编程助手协作。
...
安装完成后建议运行 `lwa doctor` 自检环境（Python / Docker / Compose / 磁盘 / 内存），详见 [排障指南](docs/faq.md)。
...
├─ logs/                    # 全局日志
├─ skills/                  # 12 个大模型协作 SKILL.md（WBS-24）
└─ apps/<id>/
...
代码位于 `src/local_web_access/`，测试位于 `tests/`。容器相关测试通过替身（fake runtime）运行，无需本机安装 Docker；真实 Docker 集成测试需设置 `LWA_RUN_DOCKER_TESTS=1`，详见 [测试指南](docs/testing.md)。
