...
# 7b.（可选，macOS）登录自启 daemon + manager（+ 可选 caddy）
lwa setup --autostart         # 生成 launchd plist，见 docs/autostart.md
# lwa setup --autostart --with-caddy
...
| `lwa remove --redundant [--purge]` | 批量清理冗余实例（按 `sourceZipHash` 去重保留最早者，IMP-012） |
...
| `lwa list` | 列出所有实例及端口 |
| `lwa setup [--script] [--json] [--autostart] [--with-caddy]` | 检测宿主机工具环境；`--autostart` 生成 macOS launchd 开机自启 plist（OPS-025） |
...
静态实例路径别名设置、pending/failed/可恢复异常队列、端口池与统计。Caddy 模式下实例状态除
运行中/已停止外，还会区分 **网关不可达**（`gateway_down`，master 离线）与 **配置无效**
（`config_invalid`，master 在线但站点端口不通）；二者在列表中高亮，可点 **恢复**（先尝试
拉起 Caddy master 再 restart，等价 `POST /api/instances/{id}/recover`）。API 端点与鉴权细节见
[管理页说明](docs/manager-page.md)。
