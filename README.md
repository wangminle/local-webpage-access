...
| `lwa ca export [--out PATH]` | Export the internal-CA root cert with SHA-256 fingerprint + per-OS trust instructions (gatewayTls=internal; V0.8.19) |
...
| `lwa ca export [--out PATH]` | 导出内部 CA 根证书 + SHA-256 指纹 + 各平台信任指引（gatewayTls=internal；V0.8.19） |
