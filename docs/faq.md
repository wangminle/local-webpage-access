...
回滚收尾、daemon 自愈重建都会短时间占锁。V0.8.8+ 的报错按持有者状态分流：

* **持有者存活且心跳仍在刷新**（如 daemon 正在执行长构建，可持续数十分钟）：
  提示"请耐心等待该操作完成，无需手动重试"——这是正常进行中的操作，不是
  死锁，不要 kill 持有者；
* **持有者存活但心跳停滞**（超过 1800 秒观测阈值）：疑似持有者卡死，可排查
  并在确认无有效工作后**安全终止持有进程**（其文件锁随进程退出自动释放），
  再重试本操作。**不要手工删除 `run/lifecycle-<id>.lock`**——会破坏锁互斥
  （BUG-213 同 inode 设计），且项目没有 unlock 命令；
* **持有者进程已退出**：文件锁已随进程退出释放，稍候重试即可接管。

另外 daemon 对**刚失败**的实例会先退避一个周期（默认 60s）再自动重建，避免
与手工 `lwa restart` 相撞。daemon 自动恢复的起止与 `lifecycle_stage` 阶段
会镜像一份到 `logs/lwa.log`（完整日志在 `logs/daemon.log`，issue #16），
构建失败后无需手动重试，盯 `lwa.log` 即可看到自动恢复进度。
