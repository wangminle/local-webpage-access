...
* **环境变量**：生成的 `.env` 仅含端口与资源限额；应用所需业务环境变量需用户在 `docker/.env` 手动追加。
...
* **批量操作**：无内置批量 start/stop；需借助 shell 循环或管理页 API。
* **滚动更新**：不支持蓝绿/滚动发布，`rebuild` 是停机重建。
* **CI/CD 集成**：无原生 webhook 触发器；可通过 inbox/ + daemon 或 API 自行实现。

## 大模型 Skills

* 12 个 SKILL.md 覆盖常见场景，但**不保证**特定 AI 工具能正确消费；
